# app.py - Frontend do Chatbot Quadra (OK e corrigido)
...
def extract_name_from_email(email):
    ...
    parts = re.sub(r'[\._]', ' ', local_part).split()
...
def render_login_screen():
    ...
    .login-stack [data-testid="stTextInput"] input{
      ...
      border:1px solid rgba(255,255,255,.2) !important; background:#ffffff !important; color:#111827 !important;
      box-shadow:0 6px 20px rgba(6,16,35,.30);
    }
    /* Link cadastro (menos destaque) */
    ...
    @media (max-width:480px){
      :root{ --lift:28px; } .login-logo{ width:76px; height:76px; } .login-title{ font-size:1.4rem; }
    }
    </style>
    """, unsafe_allow_html=True)

    # 1 coluna central para tudo
    ...
    with mid:
        ...
        # Botão ENTRAR centralizado exatamente sob o input
...
# ===== CSS do chat (restaurado e completo) =====
st.markdown(f"""
<style>
...
:root{{
  ...
  --bg:#1C1F26; --panel:#0B0D10; --panel-header:#14171C; --border:#242833;
...
.user-circle{{ width:32px; height:32px; border-radius:50%; background:#007bff; color:#fff; display:flex; align-items:center; justify-content:center; font-weight:600; font-size:1rem; }}
...
section[data-testid="stSidebar"]{{
  ...
  margin:0 !important; padding:0 !important; background:var(--panel) !important; border-right:1px solid var(--border); z-index:900 !important;
  transform:none !important; visibility:visible !important; overflow:hidden !important; color:var(--text);
}}
...
.sidebar-header{{ font-size:1.1rem; font-weight:700; color:var(--text); margin:0 4px -2px 2px }}
...
#chatCard, .chat-card{{
  position:relative; z-index:50 !important; background:var(--bg) !important; border:none !important;
  ...
  padding-bottom:var(--chat-safe-gap); scroll-padding-bottom:var(--chat-safe-gap); color:var(--text);
}}
.message-row{{ display:flex !important; margin:12px 4px; scroll-margin-bottom:calc(var(--chat-safe-gap) + 16px) }}
.message-row.user{{ justify-content:flex-end }}
.message-row.assistant{{ justify-content:flex-start }}
.bubble{{ max-width:88%; padding:14px 16px; border-radius:12px; font-size:15px; line-height:1.45; color:var(--text); border:1px solid transparent !important; }}
...
[data-testid="stChatInput"] > div{{
  background:var(--input-bg) !important; border:1px solid var(--input-border) !important; border-radius:999px !important;
  box-shadow:0 10px 24px rgba(0,0,0,.35) !important; overflow:hidden;
}}
[data-testid="stChatInput"] textarea{{
  width:100% !important; border:none !important; border-radius:999px !important; padding:18px 20px !important; font-size:16px !important;
  outline:none !important; min-height:44px !important; max-height:220px !important; overflow-y:hidden !important; caret-color:#fff !important;
}}
[data-testid="stBottomBlockContainer"]{{ padding:0 !important; margin:0 !important; height:0 !important; min-height:0 !important; }}
*::-webkit-scrollbar{{ width:10px; height:10px }} *::-webkit-scrollbar-thumb{{ background:#2C3340; border-radius:8px }}
...
<div class="header">
  <div class="header-left">
    {logo_img_tag}
    <div>Chatbot Quadra<div class="title-sub" style="font-weight:500;font-size:.85rem;color:#9AA4B2">Assistente Inteligente</div></div>
  </div>
  <div class="header-right">
    <a href="?logout=1" target="_self" style="text-decoration:none;background:transparent;border:1px solid rgba(255,255,255,0.14);color:#e5e7eb;font-weight:600;padding:8px 12px;border-radius:10px;display:inline-block;cursor:pointer;">Sair</a>
