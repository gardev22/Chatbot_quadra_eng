...
def render_login_screen():
    """Login centralizado (logo → título → subtítulo → input → botão → disclaimer), input menor no MEIO, sem 'Press Enter...'."""
    st.markdown("""
    <style>
    /* Fundo azul (um pouco mais escuro no centro p/ realçar o logo) */
    .stApp{
        background: radial-gradient(1000px 560px at 50% 34%,
                    #2f56d2 0%, #2748b9 42%, #172c66 76%, #0a1224 100%) !important;
    ...
    /* Wrapper visual (sem card) */
    .login-wrap{ text-align:center; color:#dbe6ff; }
    .login-logo{ width:64px; height:64px; object-fit:contain; margin:0 auto 14px; filter: drop-shadow(0 2px 6px rgba(0,0,0,.35)); }
    .login-title{ font-size:1.6rem; font-weight:800; color:#ffffff; margin:4px 0 8px; text-shadow:0 1px 2px rgba(0,0,0,.25); }
    ...
    /* Esconde qualquer instrução do input */
    [data-testid="InputInstructions"], [aria-live="polite"], small{ display:none !important; visibility:hidden !important; }

    /* Estilo do input dentro da coluna central */
    .login-input input{
    ...
    .login-btn > button:hover{ filter:brightness(1.08); }
    </style>
    ...
    # --- UI (mesma disposição, centralizada)
    with st.container():
        ...
        # Input MENOR no MEIO (usa columns para limitar largura)
        c1, c2, c3 = st.columns([3, 4, 3])   # coluna central = ~40% da largura
        with c2:
            # marca classe CSS na div do input
