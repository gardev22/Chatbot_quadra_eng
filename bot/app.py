...
# ====== CSS GLOBAL MUITO CEDO (evita flash do Q gigante / fundo branco) ======
st.markdown("""
<style>
/* Fundo escuro imediato (antes de qualquer outra folha) */
html, body, .stApp { background: #0B1730 !important; }

/* Qualquer logo com esse data-attr nasce no tamanho certo mesmo sem CSS */
img[data-quadra-logo="1"] { height: 88px !important; width: auto !important; display:block; margin:0 auto 14px; }
img[data-header-logo="1"] { height: 44px !important; width:auto !important; display:inline-block; }
</style>
...
# Logo do topo com atributos width/height e data-attr (sem “flash”)
if logo_b64:
    logo_img_tag = (
        f'<img alt="Logo Quadra" data-header-logo="1" '
        f'width="44" height="44" '
        f'src="data:image/png;base64,{logo_b64}" class="logo" />'
    )
else:
...
def render_login_screen():
    ...
    .login-stack{
        width:min(92vw, var(--login-max)); margin:0 auto; text-align:center; transform: translateY(calc(var(--lift) * -1));
    }
    .login-title{
        display:block; text-align:center; font-size:1.5rem; font-weight:800; letter-spacing:.2px;
    ...
    .login-actions .stButton > button{
        ...
        background:#2E5CB5 !important; color:#ffffff !important; margin-top:12px; box-shadow:0 8px 22px rgba(11,45,110,.45);
    }
    ...
    .login-stack > div{ background: transparent !important; border: none !important; box-shadow: none !important; outline: none !important; padding: 0 !important; }
    ...
    with col_mid:
        ...
        if logo_b64:
            # Logo com atributos width/height + data-attr (sem flash gigante)
            st.markdown(
                f'<img alt="Logo Quadra" data-quadra-logo="1" width="88" height="88" '
